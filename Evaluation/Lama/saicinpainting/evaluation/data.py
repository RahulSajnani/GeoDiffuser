...
class GeodiffInpaintingDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        mask = load_image(self.mask_filenames[i], mode='L')
        if self.dilate_mask:
            print("[INFO]: Dilating mask by 10")
...
class Zero123InpaintingDataset(Dataset):
    def __init__(self, datadir, img_suffix='.jpg', pad_out_to_modulo=None, scale_factor=None, dilate_mask = False):
        self.datadir = datadir
        self.folder_list = glob.glob(complete_path(self.datadir) + "**/")
        self.folder_list.sort()
        self.mask_filenames = []
        ...
        for f in self.folder_list:
            if file_exists(f + "zero123/blended.png"):
                self.img_filenames.append(f + "zero123/blended.png")
                self.mask_filenames.append(f + "zero123/inpainting_mask.png")
