...
def read_pickle(pickle_file, read_type="rb"):

    f = open(pickle_file, read_type)
    out = pickle.load(f)
    f.close()
...
def process_depth(depth, bits = 2):


    print("processing depth and bringing it to appropriate scale")
    depth_min = depth.min()
    depth_max = depth.max()
